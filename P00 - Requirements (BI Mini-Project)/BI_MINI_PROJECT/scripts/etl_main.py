...
DATA_DIR = '/data'
BATCH_SIZE = 20000  # Records per batch for memory efficiency
...
def main():
    ...
    log("=" * 60)
    log("FRAUD DETECTION ETL PIPELINE")
    log("=" * 60)
    ...
    try:
        ...
        log("")
        log("Next Steps:")
        log("  1. Verify data: docker exec bi_postgres psql -U postgres -d fraud_detection -c '\\dt'")
        log("  2. Check counts: docker exec bi_postgres psql -U postgres -d fraud_detection -c 'SELECT COUNT(*) FROM fact_transactions;'")
